...
- **`nextFocus` is content ↔ backlinks only.** The tree is no longer a Tab destination — it lives in a modal. The cycle skips backlinks when its pane isn't visible (`shouldShowBacklinks`). Don't add `focusTree` back; the modal-toggle path (`^b`) is the only way into the tree, and you wouldn't want Tab to drop into a fullscreen modal anyway.
