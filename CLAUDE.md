...
- **Tree visibility has two states: intent vs effective.** `m.tree.visible` is what the user wants (toggled by `^b`); `m.shouldShowTree()` is what's actually rendered. The latter additionally requires `m.width >= twoPaneMinWidth` (80 cols). Below the threshold, `^b` flips the intent silently — preserved for when the terminal grows back. This mirrors `m.backlinks.open` (intent) / `shouldShowBacklinks()` (effective, gated on height). Layout code reads `shouldShowTree()`; only `^b` writes `tree.visible`. The tree starts **hidden** (`tree.visible: false`, `focus: focusContent`) — the `^p` finder is the primary navigation surface; the tree is opt-in.
- **Mouse routing respects `shouldShowTree()`.** `handleMouse` skips tree-row hit-testing when the tree pane isn't on screen. BubbleZone keeps zone bounds across re-renders, so a hidden tree's stale row zones would otherwise still catch clicks (this surfaced as cross-test pollution when one test rendered with the tree visible and a subsequent test clicked on a content link at the same coordinates). The model retains `tree.flat` / `tree.cursor` so the tree can be revealed cheaply via `^b`, but anything that *acts* on a tree row must gate on visibility.
- **`nextFocus` skips hidden panes.** Tab cycles `focusContent → focusTree → focusContent` only when the tree is shown, and through `focusBacklinks` only when the backlinks pane is shown. Don't add a destination to the cycle without checking the corresponding `shouldShowX()` — otherwise Tab will land on an invisible pane and the after-the-fact guard in `content.go` will visibly snap focus back, which feels broken.
