...
- **`^p` opens a `bubbles/filepicker` modal as a fourth `modalKind`.** It reuses `renderModal`/`overlayModal` but supplies its own body (`m.picker.View()` instead of `m.modalVP.View()`). At the vault root, Esc closes; deeper, filepicker's native Esc-walks-up behavior takes over (this is intentional — filepicker treats Esc as `Back`). The top-level `Update` forwards non-key messages (notably `readDirMsg` from `m.picker.Init()`) to the picker while it's open; without that forwarding, the picker can't load its directory listing.
