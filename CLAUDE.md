...
- **`^block-id` markers are stripped from rendered output.** The vault index sees the raw marker (so `[[Note#^foo]]` resolves), but `preprocessBlockMarkers` (in `internal/markdown/links_render.go`) removes the trailing ` ^id` from every non-fence line before Glamour renders. Markers inside fenced code blocks render verbatim. Duplicate ids in the same file: first wins, with a `warn` diagnostic via the existing `Diagnostics` stream.
...
**Wikilinks and backlinks — Phase 2 in progress:** inline-link pre-selection on backlink-follow / Back / Forward shipped (see [pre-select-inline-link](docs/superpowers/specs/2026-05-09-pre-select-inline-link-design.md)). Broken-link tally in the footer shipped (see [broken-link-tally](docs/superpowers/specs/2026-05-25-broken-link-tally-design.md)). Heading and block anchors shipped (see [block-references](docs/superpowers/specs/2026-05-25-block-references-design.md)). Remaining: configurable vault root. Design outlined in [docs/superpowers/specs/2026-05-07-wikilinks-and-backlinks-design.md](docs/superpowers/specs/2026-05-07-wikilinks-and-backlinks-design.md).
