...
- [MCP server](superpowers/specs/2026-06-22-mcp-server-design.md) — draft — a `hypogeum mcp [vault]` subcommand serving the vault to Claude/agents over MCP (stdio). Thin handlers over `internal/query` plus a watcher-refreshed warm `vault.Build` index (the amortization that justifies a server vs. the per-call CLI verbs). Tools: `search_vault`, `outbound_links`, `neighbors`, `vault_graph`, `read_note`; read-only, no TUI dependency.
