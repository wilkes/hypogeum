...
- [Keyboard selection (vim visual mode)](superpowers/specs/2026-06-13-keyboard-selection-design.md) — designed — `v` enters a vim-style visual mode (both dialects) that reuses the mouse selection's `selection{anchor,cursor}` span machinery; `h/j/k/l`/arrows + `g`/`G` + `^d`/`^u` move the caret, the dialect's copy key yanks, `Esc` cancels.
