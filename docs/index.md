...
- [Split recency signals](superpowers/specs/2026-06-20-split-recency-signals-design.md) — pull the blended mtime+visit score apart: finder (`^p`/`o`) and search re-rank become pure edit-recency (mtime), and a new `r` "recently opened" modal + the CLI `recent` verb become pure visit-recency (visited-only, last-visited first). Removes the decay blend, weights, and `Ranked.Score`.
- [Default index/readme landing file](superpowers/specs/2026-06-20-default-index-readme-design.md) — when opening a directory with no explicit file, prefer a top-level `index.*` then `readme.*` (case-insensitive stem) before falling back to the first file. Top-level/markdown-only.
...
- [Benchmarking](benchmarking.md) — how to run the hot-path benchmarks (`internal/benchcorpus` corpus + per-package `*_bench_test.go`) and the latest findings, including the 100k–1M extreme-scale sweep and the macOS vnode-cache cliff behind `recent.Rank`'s super-linear scaling.
