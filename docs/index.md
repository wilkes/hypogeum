...
- [Keyboard selection (vim visual mode)](superpowers/specs/2026-06-13-keyboard-selection-design.md) — shipped — two-phase vim-style visual mode (both dialects) reusing the mouse selection's `selection{anchor,cursor}` span machinery: `v` shows a movable caret, `Space` drops the anchor to start extending, `h/j/k/l`/arrows + `g`/`G` + `^d`/`^u` move it, the dialect's copy key yanks, `Esc` cancels.
