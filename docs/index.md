...
- [Architecture overview](architecture.md) — package layering, data flow on a keystroke, where to start reading
  ...
  - `internal/query` — non-interactive JSON query mode (`search`/`links`/`recent`/`neighbors`/`graph`), no TUI deps
  - `internal/mcp` — MCP server (stdio) exposing the query surface as agent tools, over a warm watcher-refreshed vault index
...
- [`graph` query verb](superpowers/specs/2026-06-20-graph-verb-design.md) — design approved 2026-06-20 — non-interactive `hypogeum graph [--vault DIR]` emitting the whole-vault link graph as JSON (`{nodes, edges}`). Nodes are every markdown doc (orphans included); edges are all four link kinds with brokenness, reusing `outboundLinks`.
...
- [Search file grouping](superpowers/specs/2026-06-22-search-file-grouping-design.md) — draft — rework the `/` search modal into a file-grouped view: one row per matching file with a match **count** + proportional bar, sorted count-descending, individual matches reachable by inline expand/collapse (`Tab`). Adds a lazy-snippet `search.SearchGrouped` (count all matches, build snippets only for expanded files) so the count is accurate and the collapsed case is cheaper than today. Goal: scan far more files at a glance.
...
- [MCP server](superpowers/specs/2026-06-22-mcp-server-design.md) — shipped — `hypogeum mcp [vault]` serves the vault to Claude/agents over MCP (stdio). Thin handlers over `internal/query` plus a watcher-refreshed warm `vault.Build` index (the amortization that justifies a server vs. the per-call CLI verbs). Tools: `search_vault`, `outbound_links`, `neighbors`, `vault_graph`, `read_note`; read-only, no TUI dependency. (Adds the official Go MCP SDK and bumps the toolchain to Go 1.25.)
- [hypogeum-vault skill](superpowers/specs/2026-06-20-hypogeum-vault-skill-design.md) — design — an agent-facing Claude skill (`.claude/skills/hypogeum-vault/`) that teaches exploring and auditing a markdown vault via the query CLI (`neighbors`/`links`/`search`/`recent`) instead of grep, including whole-vault broken-link and orphan sweeps. Lives in-repo; symlink into `~/.claude/skills/` for global use. [Plan](superpowers/plans/2026-06-20-hypogeum-vault-skill.md).
