...
- [Search file grouping](superpowers/specs/2026-06-22-search-file-grouping-design.md) — shipped — the `/` search modal is now a file-grouped view: one row per matching file with a match **count** + proportional bar, sorted count-descending (tie-break recency), individual matches reached by inline expand/collapse (`Tab`). Backed by a lazy-snippet `search.SearchGrouped` (count all matches, build snippets only for expanded files) so counts are accurate and the collapsed case is cheaper than the old per-hit list.
